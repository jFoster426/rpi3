...
const unsigned char font[32 * 3][7] = {
...
void console_init(unsigned int x, unsigned int y) {
	framebuffer_init(x, y, 32);
	framebuffer_allocate();
	cur_x = 0;
	cur_y = 0;
}

void console_putc(unsigned char c, unsigned int color) {
	if(cur_x > framebuffer.screen_width + 5) {
		cur_y += 10;
		cur_x = 0;
	}
	if(cur_y > framebuffer.screen_height + 7) {
		cur_y = 0;
	}
	if(c == '\n') {
		cur_y += 10;
	...
	for(y = 0; y < 7; y++) {
		for(x = 0; x < 5; x++) {
			pixel = ((font[c][y] >> (4 - x)) & 1) * color;
			framebuffer_write(((x + cur_x) * 4) + ((y + cur_y) * framebuffer.pitch), pixel);
		}
	}
	cur_x += 6;
}

void console_puts(char *s, unsigned int color) {
	while(*s != '\0') {
...
void console_clear() {
	...
	for(y = 0; y < cur_y; y++) {
		for(x = 0; x < cur_x; x++) {
			framebuffer_write((x * 4) + (y * framebuffer.pitch), 0x00000000);
		}
	}
	cur_x = 0; cur_y = 0;
}

void console_setposition(unsigned int x, unsigned int y) {
	cur_x = x;
	cur_y = y;
}
